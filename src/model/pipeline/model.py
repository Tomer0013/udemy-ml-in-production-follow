...
def build_model() -> None:
    ...
    # 6. Persist trained model to disk
    _save_model(rf)
...
def _get_x_y(
    ...
    """
    logger.info(f"Defining X and y vars. \nX vars: {col_x}\ny var: {col_y}")
...
def _train_model(X_train: pd.DataFrame, y_train: pd.Series) -> RandomForestRegressor:
    ...
    logger.debug(f"Using grid space: {grid_space}")
    grid = GridSearchCV(RandomForestRegressor(), param_grid=grid_space, cv=3, scoring="r2")
...
def _save_model(model: RandomForestRegressor) -> None:
    ...
    Return:
        None
    """
    model_save_path = Path(model_settings.model_path) / model_settings.model_name
    logger.info(f"Saving a model to directory {model_save_path}")
